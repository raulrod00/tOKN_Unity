...
namespace UXF
{
    ...
    public class Session : MonoBehaviour, ISettingsContainer, IDataAssociatable
    {
        ...
        /// </summary>
        /// <returns></returns>
        public void SetCurrentTrial(int trialNumber)
        {
        ...
        public void LoadPausedSession()
        {
            ...
            int ctr = 0;
            string[] out_values = new string[12];

            using (var reader = new StreamReader(f_String.getName()))
            {
                while (!reader.EndOfStream)
                {
                    ...
                    if (ctr == 1)
                    {
                        f_String.setExpName(values[0]);
                        f_String.setPPIDName(values[1]);
                        f_String.sessionNum(int.Parse(values[2]));
                    }
                    ctr += 1;
                    out_values = values;
                }
                f_String.listHandler(out_values[out_values.Length-1]);
                f_String.currTrials(int.Parse(out_values[3]));
            }
        }
